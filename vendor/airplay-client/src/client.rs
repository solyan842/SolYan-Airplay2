...
impl AirPlayClient {
    ...
    pub async fn play_file_to_group(&mut self, path: impl AsRef<Path>) -> Result<()> {
        ...
        if let Some(ref mut conn) = self.connection {
            conn.send_flush(0, 0).await?;
        }
        for conn in self.group_connections.iter_mut() {
            conn.send_flush(0, 0).await?;
        }
        // Re-send RECORD after FLUSH to re-arm every device. A group is only
        // considered ready when every member has acknowledged the command.
        if let Some(ref mut conn) = self.connection {
