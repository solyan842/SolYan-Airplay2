...
fn set_socket_qos(_socket: &UdpSocket) {
    // AirPlay RTP does not depend on these QoS hints for correctness.
    // Windows socket tuning is intentionally deferred to a WinSock-native
    // implementation; keeping this a no-op preserves the upstream RTP wire
    // protocol while making the sender portable to Windows.
    tracing::debug!("RTP QoS tuning skipped on non-Unix platform");
}
