...
enum SenderMessage {
    ...
    Packet {
        ...
        /// Monotonic time when sync/audio metadata was prepared. The sender
        /// corrects the embedded 32.32 clock timestamp for queue residence time.
...
fn advance_sync_clock(packet: &mut [u8], elapsed: std::time::Duration) {
    if packet.len() < 16 {
        return;
    }

    let payload_type = packet[1] & 0x7f;
    if payload_type != 84 && payload_type != 87 {
        return;
    }

    let mut raw = [0u8; 8];
    raw.copy_from_slice(&packet[8..16]);
    let base = u64::from_be_bytes(raw);

    let secs = elapsed.as_secs();
    let nanos = elapsed.subsec_nanos() as u64;
    let frac = ((nanos as u128) << 32) / 1_000_000_000u128;
    let delta = (secs << 32).wrapping_add(frac as u64);
    let corrected = base.wrapping_add(delta);

    packet[8..16].copy_from_slice(&corrected.to_be_bytes());
}
...
fn sender_thread_main(
    ...
    loop {
        ...
        match msg {
            ...
            SenderMessage::Packet { wire_packets, sync_data, prepared_at } => {
                // Buffer packet for burst sending
                burst_buffer.push((wire_packets, sync_data, prepared_at));
                ...
                for (wire_packets, mut sync_data, prepared_at) in burst_buffer.drain(..) {
                    // Sync timestamps are generated by the producer before the
                    // bounded sender queue. Correct the embedded 32.32 timestamp
                    // by the actual queue residence time immediately before send.
                    // This keeps RTP<->clock mapping fresh and prevents periodic
                    // receiver clock corrections that can be audible as ticks.
                    if let Some(ref mut sync) = sync_data {
                        let residence = std::time::Instant::now().saturating_duration_since(prepared_at);
                        advance_sync_clock(sync, residence);
