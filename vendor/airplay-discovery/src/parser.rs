...
impl TxtRecordParser {
    ...
    pub fn parse_airplay_txt(
        ...
        // Parse group UUID
        // Modern HomePod stereo pairs may advertise a composite gid such as
        // "<tight-sync-uuid>+1+<member/group-uuid>". Treat the first UUID as
        // the stable group root instead of rejecting the entire AirPlay TXT
        // record. Rejecting it would make discovery fall back to RAOP and lose
        // tsid/gpn/igl/gcgl, which are exactly the fields needed for stereo-pair
        // detection.
        let group_id = txt
            .get("gid")
            .map(|gid| Self::parse_group_uuid(gid))
            .transpose()?;
        ...
        let parent_group_id = txt
            .get("pgid")
            .map(|pgid| Self::parse_group_uuid(pgid))
            .transpose()?;
        ...
        let tight_sync_id = txt
            .get("tsid")
            .map(|tsid| Self::parse_group_uuid(tsid))
            .transpose()?;
    ...
    /// Parse Apple group identifiers. HomePod can publish a composite gid
    /// (for example UUID+1+UUID); the leading UUID is the stable group root.
    fn parse_group_uuid(raw: &str) -> Result<uuid::Uuid, ParseError> {
        let first = raw.split('+').next().unwrap_or(raw).trim();
        uuid::Uuid::parse_str(first)
            .map_err(|_| ParseError::InvalidValue(format!("invalid group UUID: {}", raw)))
    }
...
mod tests {
    ...
    mod parse_airplay_txt {
        ...
        #[test]
        fn parses_composite_homepod_group_uuid() {
            let txt = make_txt(&[
                ("deviceid", "94:EA:32:88:34:6C"),
                ("model", "AudioAccessory5,1"),
                ("gid", "838D11F4-A98F-5A37-BE92-48BF07D79E58+1+5F816FBE-FBB0-4925-B1A6-A5A3194042C9"),
                ("tsid", "838D11F4-A98F-5A37-BE92-48BF07D79E58"),
                ("gpn", "Phòng ngủ"),
                ("igl", "1"),
                ("gcgl", "1"),
            ]);
            let device = TxtRecordParser::parse_airplay_txt("Phòng ngủ", &txt, vec![], 7000).unwrap();
            assert_eq!(
                device.group_id.unwrap().to_string(),
                "838d11f4-a98f-5a37-be92-48bf07d79e58"
            );
            assert_eq!(
                device.tight_sync_id.unwrap().to_string(),
                "838d11f4-a98f-5a37-be92-48bf07d79e58"
            );
            assert_eq!(device.group_public_name.as_deref(), Some("Phòng ngủ"));
        }
